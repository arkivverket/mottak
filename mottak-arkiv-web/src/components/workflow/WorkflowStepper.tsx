	...
	return (
		<StepperContext.Provider value={{ handleNext, handleCancel }}>
			<Grid container direction="column" justify="space-around" alignItems="center">
				<Grid item>
					<Stepper alternativeLabel activeStep={activeStep} style={{ width: '50vw' }} data-testid="stepper">
						{steps?.map((step) => (
							<Step key={step.number}>
								<StepLabel className={classes.root}>{step.label}</StepLabel>
							</Step>
						))}
					</Stepper>
				</Grid>
				<Grid item>{step?.component}</Grid>
			</Grid>
